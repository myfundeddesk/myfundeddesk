...
def seed_database():
    ...
    packages = [
        ...
        ChallengePackage(
            ...
            min_trading_days=3,
            price=3999,
            description="Profit Target: 10% | Daily Loss Limit: 3% | Max Drawdown: 6%"
        ),
        ChallengePackage(
            name="Executive",
            model_type="1-Step",
    ...
    for pkg in packages:
        if not db.query(ChallengePackage).filter(
            ChallengePackage.name == pkg.name,
            ChallengePackage.model_type == pkg.model_type
        ).first():
            db.add(pkg)
    db.commit()
    db.close()
